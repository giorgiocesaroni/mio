...
def get_quick_log_prompt(
    ...
    if mode == "edit":
        task = (
            ...
            "then apply the correction with `update_log`, `delete_log`, or additional `log_ingredient`/`log_recipe` calls. "
    ...
    rules = (
        ...
        "- ALWAYS finish with at least one log mutation (`log_ingredient`, `log_recipe`, `update_log`, or `delete_log`). Logging something approximate is better than logging nothing.\n"
